...
if [ "$OS" = "windows" ]; then
    go build -o dist/bin/prompt-wizard.exe ./src/prompt-wizard.go
...
if [ -f "src/prompt-huh.go" ]; then
    echo "   Building prompt-huh..."
    if [ "$OS" = "windows" ]; then
        go build -o dist/bin/prompt-huh.exe ./src/prompt-huh.go 2>/dev/null || echo "   ⚠️  prompt-huh build skipped (optional)"
    else
        go build -o dist/bin/prompt-huh ./src/prompt-huh.go 2>/dev/null || echo "   ⚠️  prompt-huh build skipped (optional)"
    fi
fi
